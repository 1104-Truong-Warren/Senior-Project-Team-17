// From Drag Camera 2D package
// some modifications for tutorial use
// - Ellison
...
public class DragCamera2D : MonoBehaviour
{
    ...
    */
    ...
    // Ellison - added to mark if camera actions have been done for tutorial
    public bool hasDonePan = false;
    public bool hasDoneZoom = false;
...
;
    ...
    public void panControl() {
        ...
        if (keyboardInput) {
            ...
            transform.Translate(x, y, 0);
            hasDonePan = true; // tutorial detection
        }
    ...
    public void zoomControl() {
        if (zoomToMouse) {
            if (Input.GetAxis("Mouse ScrollWheel") > 0 && minZoom < Camera.main.orthographicSize) // forward
            {
                ...
                hasDoneZoom = true; // tutorial detection
            }
